...
type FileSystem interface {
	...
	UnmountLocal(ctx context.Context, mountpoint string) error
	MountLocal(ctx context.Context, mountpoint string, labels map[string]string, mounts []mount.Mount) error
	IDMapMount(ctx context.Context, mountpoint, activeLayerKey string, idmap idtools.IDMap) (string, error)
	IDMapMountLocal(ctx context.Context, mountpoint, activeLayerKey string, idmap idtools.IDMap) (string, error)
}
...
func (o *snapshotter) setupIDMap(ctx context.Context, s storage.Snapshot, parent string, labels map[string]string) error {
	...
	if err != nil {
		log.G(ctx).Errorf("failed to load id-map: %v", err)
		return err
	}
	...
	if !idmap.Empty() {
		...
		if err != nil {
			log.G(ctx).Errorf("failed to stat parent snapshot: %v", err)
			return err
		}
		...
		if _, ok := parentSnapshot.Labels[source.HasSociIndexDigest]; !ok {
			// Fallback to overlay
			log.G(ctx).Info("no SOCI index found, remapping from root")
			...
			if err != nil {
				log.G(ctx).Errorf("failed to create id-mapped mounts: %v", err)
				return err
			}
		}

		log.G(ctx).Info("id-mapping successful")
	}
	return nil
}
...
func (o *snapshotter) getCleanupDirectories(ctx context.Context, t storage.Transactor, cleanupCommitted bool) ([]string, error) {
	...
	for _, d := range dirs {
		if !cleanupCommitted {
			currDir := d
			// Use the id-mapped suffix if present
			temp := strings.Split(d, "_")
			if len(temp) > 1 {
				currDir = temp[1]
			}
			if _, ok := ids[currDir]; ok {
				continue
			}
		}
...
func (o *snapshotter) unmountSnapshotDirectory(ctx context.Context, dir string) error {
	...
	if mounted {
		if err := o.fs.Unmount(ctx, mp); err == errdefs.ErrNotFound {
			return o.fs.UnmountLocal(ctx, mp)
		}
		return err
	}
	return nil
...
func (o *snapshotter) mounts(ctx context.Context, s storage.Snapshot, checkKey string) ([]mount.Mount, error) {
	...
	parentPaths, err := o.getParentPaths(ctx, s)
	if err != nil {
		return nil, err
	}
